...
func getHTML(url string) (HTML string, err error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to build request,error %w", err)
	}
	...
	if resp.StatusCode > 299 {
		return "", fmt.Errorf("page (%v) didn't return a 200 OK or similar, status code: %v, error : %w", url, resp.StatusCode, err)
	}
